...
int main()
{
    ...
    ALLEGRO_FONT* fontmenor = al_load_font("arial.ttf", 18, 0);
    must_init(font, "font");
    must_init(fontmenor, "fontmenor");

    must_init(al_init_image_addon(), "image addon");
    ALLEGRO_BITMAP* agua = al_load_bitmap("agua.jpeg");
    ALLEGRO_BITMAP* heart = al_load_bitmap("heart.jpg");
    ALLEGRO_BITMAP* zumbi = al_load_bitmap("zumbi.jpg");
    must_init(agua, "agua");
   ...
   for(int i = 0; i < FRASES; i++){
    ...
    while(1)
    {
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            al_draw_text(font, al_map_rgb(255, 0, 0), x[0], 50, 1, "FASE 1");
            al_draw_text(font, al_map_rgb(0, 0, 0), x[1], 150, 1, "MATE 100 ZUMBIS");
            al_draw_bitmap(zumbi, x[1]-40, 190, 0);
            al_draw_text(font, al_map_rgb(0, 0, 0), x[2], 340, 1, "VOCÊ TEM 3 VIDAS");
            al_draw_bitmap(heart, x[2]-90, 380, 0);
            al_draw_bitmap(heart, x[2]-30, 380, 0);
            al_draw_bitmap(heart, x[2]+30, 380, 0);
            al_draw_text(font, al_map_rgb(0, 0, 0), x[3], 490, 1, "CONSIGA ÁGUA");
            al_draw_bitmap(agua, x[3]-25, 530, 0);
            al_draw_text(font, al_map_rgb(255, 0, 0), x[4], 591, 1, "BOA SORTE !");
