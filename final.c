...
int main()
{
    ...
    ALLEGRO_BITMAP* reacao = al_load_bitmap("reacao.jpg");

    must_init(reacao, "reacao");
   ...
   for(int i = 0; i < FRASES; i++){
    ...
    while(1)
    {
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            al_draw_text(font, al_map_rgb(255, 0, 0), x[0], 150, 1, "AGORA VOCÊ TEM ÁLCOOL PROPÍLICO");
            al_draw_bitmap(reacao, x[1]-111, 200, 0);
            al_draw_text(font, al_map_rgb(255, 0, 0), x[2], 500, 1, "E PODE SE DESCOLAR !");
