...
int main()
{
    ...
    ALLEGRO_BITMAP* agua = al_load_bitmap("agua.jpeg");
    ALLEGRO_BITMAP* propeno = al_load_bitmap("prope.png");
    ALLEGRO_BITMAP* acido = al_load_bitmap("acido.jpg");
    must_init(agua, "agua");
    must_init(propeno, "propeno");
   ...
   for(int i = 0; i < FRASES; i++){
    ...
    while(1)
    {
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            al_draw_text(font, al_map_rgb(0, 0, 0), x[0], 50, 0, "A SUA CIDADE FOI ATACADA POR ZUMBIS...");
            al_draw_text(font, al_map_rgb(0, 0, 0), x[1], 130, 1, "...JUNTO DELES, VEIO UMA AVALANCHE DE SUPER COLA");
            al_draw_text(font, al_map_rgb(0, 0, 0), x[2], 210, 2, "E VOCÊ FICOU COLADE NO CENTRO DA PRAÇA");
            al_draw_text(font, al_map_rgb(0, 0, 0), x[3], 290, 0, "CONSIGA AS SEGUINTES SUBSTÂNCIAS:");
            al_draw_text(font, al_map_rgb(0, 0, 0), x[4], 370, 1, "ÁGUA");
            al_draw_bitmap(agua, x[4] + 30, 370, 0);
            al_draw_text(font, al_map_rgb(0, 0, 0), x[5], 450, 1, "PROPENO");
            al_draw_bitmap(propeno, x[5] + 50, 450, 0);
            al_draw_text(font, al_map_rgb(0, 0, 0), x[6], 530, 1, "ÁCIDO SULFÚRICO");
            al_draw_bitmap(acido, x[6] + 90, 530, 0);
            al_draw_text(font, al_map_rgb(0, 0, 0), x[7], 610, 1, "PARA FORMAR O SOLVENTE DE COLA, ÁLCOOL PROPÍLICO, E SAIR DESSA.");
            //for(int i = 0; i < BT_N; i++)
            //{
                //BOUNCER* b = &obj[i];
                //switch(b->type)
