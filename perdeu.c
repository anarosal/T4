...
int main()
{
    ...
    al_init_font_addon();
    al_init_ttf_addon();
    ALLEGRO_FONT* font = al_load_font("arial.ttf", 50, 0);
    must_init(font, "font");

    must_init(al_init_image_addon(), "image addon");
    ALLEGRO_BITMAP* zumbis = al_load_bitmap("zumbis.jpeg");

    must_init(zumbis, "zumbis");
   ...
   for(int i = 0; i < FRASES; i++){
    ...
    while(1)
    {
        ...
        if(redraw && al_is_event_queue_empty(queue))
        {
            ...
            al_draw_text(font, al_map_rgb(255, 0, 0), x[0], 250, 1, "GAME OVER");
            al_draw_bitmap(zumbis, x[1]-150, 350, 0);
